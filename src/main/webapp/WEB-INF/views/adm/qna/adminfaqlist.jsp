			...
			<div class="content">
				<div class="panel-header bg-primary-gradient">
					<div class="page-inner py-5">
						<div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
							<div>
								<h2 class="text-white pb-2 fw-bold">FAQ 관리</h2>
				...
				<div class="page-inner mt--5">
					...
					<div class="row row-card-no-pd">
					    ...
					    <c:choose>
					    	...
					    	<form>
							    ...
							    <tr>
							    	...
							    	<td>
							    	<c:choose>
							    	<c:when test="${list.cate eq 'air'}">항공권</c:when>
							    	<c:when test="${list.cate eq 'hotel'}">숙박</c:when>
							    	<c:when test="${list.cate eq 'package'}">패키지</c:when>
							    	<c:when test="${list.cate eq 'ticket'}">입장권</c:when>
							    	<c:when test="${list.cate eq 'reserv'}">상품예약.결제</c:when>
							    	<c:when test="${list.cate eq 'customer'}">회원정보</c:when>
							    	<c:when test="${list.cate eq 'etc'}">기타</c:when>
							    	</c:choose>
							    	
							    	
							    	</td>
							    	<td>
								    	<a href="/adm/qna/adminfaqmodify.do?bno=${list.bno}" class="pbtn btn btn-info">수정</a>
								    	<a href="/adm/qna/adminfaqdelete.do?bno=${list.bno}" class="pbtn btn btn-danger"  onClick="return confirm('삭제하시겠어요?');">삭제</a>
							    	</td>
							    </tr>
							    </c:forEach>
							   </form>
					  ...
					  <button onclick="location.href='/adm/qna/adminfaqwrite.do'" class="btn btn-default" style="float:right;">
					  	FAQ 등록
					  </button>
										</div>
										</div>
